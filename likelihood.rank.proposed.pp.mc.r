...
load('./data/trajectory.proposed.pp.mc.rdata')
...
load('./data/rank.proposed.pp.mc.rdata')
rank.proposed.pp.mc = rank.proposed.pp.mc$order[1:20]

trajs.proposed.pp.mc = trajs.proposed.pp.mc[ rank.proposed.pp.mc ]
rank.proposed.pp.mc = 1:20
...
f.t0.mc = mclapply(trajs.proposed.pp.mc, as.ordinal, data=d.t0.mc, exclusion=TRUE, mc.cores=mc.cores)
f.t1.mc = mclapply(trajs.proposed.pp.mc, as.ordinal, data=d.t1.mc, exclusion=TRUE, mc.cores=mc.cores)
f.t0.mc = cbind.data.frame(f.t0.mc)
f.t1.mc = cbind.data.frame(f.t1.mc)
colnames(f.t0.mc) = paste('t', 1:length(trajs.proposed.pp.mc), sep='')
colnames(f.t1.mc) = paste('t', 1:length(trajs.proposed.pp.mc), sep='')
...
# Convert a data to a set of prefixes.
f.t0.fv = mclapply(trajs.proposed.pp.mc, as.ordinal, data=d.t0.fv, exclusion=TRUE, mc.cores=mc.cores)
f.t1.fv = mclapply(trajs.proposed.pp.mc, as.ordinal, data=d.t1.fv, exclusion=TRUE, mc.cores=mc.cores)
f.t0.fv = cbind.data.frame(f.t0.fv)
f.t1.fv = cbind.data.frame(f.t1.fv)
colnames(f.t0.fv) = paste('t', 1:length(trajs.proposed.pp.mc), sep='')
colnames(f.t1.fv) = paste('t', 1:length(trajs.proposed.pp.mc), sep='')
...
errors.proposed.pp = .000
errors.proposed.pp = c(errors.proposed.pp, rep(0, length(items)-length(errors.proposed.pp)))
errors.proposed.pp = c(1-sum(errors.proposed.pp), errors.proposed.pp)
...
B = 250
nlls.rank.proposed.pp.mc = mclapply(1:B, function(b, trajs, rank, d0.mc, d1.mc, f0.mc, f1.mc, i1.mc, items, combn, errors) {
  cat('.')
  ...
  return( nlls )
}, trajs=trajs.proposed.pp.mc, rank=rank.proposed.pp.mc, d0.mc=d.t0.mc, d1.mc=d.t1.mc, f0.mc=f.t0.mc, f1.mc=f.t1.mc, i1.mc=i.t1.mc, items=items, combn=combn.vs, errors=errors.proposed.pp, mc.cores=mc.cores)

B = 250
nlls.rank.proposed.pp.fv = mclapply(1:B, function(b, trajs, rank, d0.mc, d1.mc, f0.mc, f1.mc, f0.fv, i1.fv, items, combn, errors) {
  cat('.')
  ...
  return( nlls )
}, trajs=trajs.proposed.pp.mc, rank=rank.proposed.pp.mc, d0.mc=d.t0.mc, d1.mc=d.t1.mc, f0.mc=f.t0.mc, f1.mc=f.t1.mc, f0.fv=f.t0.fv, i1.fv=i.t1.fv, items=items, combn=combn.vs, errors=errors.proposed.pp, mc.cores=mc.cores)
...
save(nlls.rank.proposed.pp.mc, nlls.rank.proposed.pp.fv, file='./data/likelihood.rank.proposed.pp.mc.rdata')

...
pool.proposed.pp = 1:length(trajs.proposed.pp.mc)
...
following.proposed.pp.mc = count.following(trajs=trajs.proposed.pp.mc, f0=f.t0.mc, f1=f.t1.mc)

fit.itms.proposed.pp.mc = mclapply(1:length(trajs.proposed.pp.mc), 
  function(k, trajs, items, logits, combn) {
    itm(k=k, trajs, items=items, logits=logits, combn=combn)
}, trajs=trajs.proposed.pp.mc, items=items, logits=logits.mc, combn=combn.vs, mc.cores=mc.cores)
...
rank.proposed.pp.mc = search.trajectories(
  f0        = f.t0.mc, 
  i1        = i.t1.mc, 
  prefix    = NULL, 
  pool      = pool.proposed.pp, 
  fit       = fit.itms.proposed.pp.mc, 
  following = following.proposed.pp.mc, 
  errors    = errors.proposed.pp, 
  mc.cores  = mc.cores
)
rank.proposed.pp.mc$order = as.integer(strsplit(x=rank.proposed.pp.mc$order[nrow(rank.proposed.pp.mc)], split='[.]')[[1]])
...
save(rank.proposed.pp.mc, file='./data/rank.proposed.pp.mc.rdata')

...
pool.proposed.ps = 1:length(trajs.proposed.ps.mc)
...
following.proposed.ps.mc = count.following(trajs=trajs.proposed.ps.mc, f0=f.t0.mc, f1=f.t1.mc)

fit.itms.proposed.ps.mc = mclapply(1:length(trajs.proposed.ps.mc), 
  function(k, trajs, items, logits, combn) {
    itm(k=k, trajs, items=items, logits=logits, combn=combn)
}, trajs=trajs.proposed.ps.mc, items=items, logits=logits.mc, combn=combn.vs, mc.cores=mc.cores)
...
rank.proposed.ps.mc = search.trajectories(
  f0        = f.t0.mc, 
  i1        = i.t1.mc, 
  prefix    = NULL, 
  pool      = pool.proposed.ps, 
  fit       = fit.itms.proposed.ps.mc, 
  following = following.proposed.ps.mc, 
  errors    = errors.proposed.ps, 
  mc.cores  = mc.cores
)
rank.proposed.ps.mc$order = as.integer(strsplit(x=rank.proposed.ps.mc$order[nrow(rank.proposed.ps.mc)], split='[.]')[[1]])
...
save(rank.proposed.ps.mc, file='./data/rank.proposed.ps.mc.rdata')
